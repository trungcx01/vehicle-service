...
export class ShopMapComponent {
  ...
  async ngOnInit(): Promise<void> {
    ...
    [this.proposal, this.proposalId, this.origin, this.destination] = await this.init();
    ...
    this.movePerStepLat =
      (this.destination_lat - this.origin_lat) / this.TOTAL_STEP;
    this.movePerStepLng =
      (this.destination_lng - this.origin_lng) / this.TOTAL_STEP;
  ...
  async init(): Promise<[number, number, string, string]> {
      ...
      this.activatedRoute.paramMap.subscribe((params) => {
        ...
        this.proposalService.getById(proposalId).subscribe({
          next: (data) => {
            ...
            resolve([data, proposalId, origin, destination]);
          },
          error: (error) => {
            console.error(error);
            reject(error);
  ...
  startShopSimulation(): void {
    const intervalId = setInterval(async () => {
      this.origin_lat += this.movePerStepLat;
      this.origin_lng += this.movePerStepLng;
      this.updateShopMarker();
      [this.fastestTime, this.shortestDistance] =
      ...
      if (this.checkIfArrived() && !this.isDialogShown) {
        ...
        Swal.fire({
          ...
          if (res.isConfirmed) {
            ...
            .updateStatus('ARRIVED', this.proposal.emergencyRequest.id)
            .subscribe({
              next: (res) => {
  ...
  updateShopMarker(): void {
    const shopMarker = this.markers.find((marker) => marker['role'] === 'SHOP');
    if (shopMarker) {
