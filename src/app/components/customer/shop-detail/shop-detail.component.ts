...
export class ShopDetailComponent implements OnInit {
  ...
  calculateRatings(): void {
    if (this.reviews && this.reviews.length > 0) {
      ...
      this.averageRating = this.reviews.reduce((sum, review) => sum + review.rate, 0) / this.totalReviews;
  ...
  async getAllAvailableVehicleCareOfShop(id: number) : Promise<any>{
    return new Promise((resolve, reject) =>{
      this.vehicleCareService.getByShopAndAvailable(id).subscribe({
        next: (response) => {
         resolve(response)
        },
        error: (err) =>{
  ...
  async ngOnInit(){
    ...
    this.activatedRoute.params.subscribe(async params => {
      const id = params['id'];
      this.vehicleCares = await this.getAllAvailableVehicleCareOfShop(id);
