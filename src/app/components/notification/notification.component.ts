...
export class NotificationComponent implements OnInit, OnDestroy {
  ...
  constructor(private notificationService: NotificationService, private toastr: ToastrService, private proposalService: ProposalService, 
    private activatedRoute: ActivatedRoute
  ) {}

  ngOnInit(): void {
      this.notificationService.connect();
      this.notificationSubscription = this.notificationService.getNotifications().subscribe((notification: string) => {
        ...
        if (notification.startsWith("PROPOSAL")){
          ...
          Swal.fire({
            ...
            if (result.isConfirmed || result.dismiss === Swal.DismissReason.timer) {
              const proposalId = notification.split(' ')[1];
